"""Агент с ролевым деревом: last_heard_msg, say из state."""
import time
...
class PlayerBot:
    def __init__(self, squad_name: str, controller: BehaviorController, protocol_version: int = 7, is_goalie: bool = False):
    ...
    def _handle_see(self, parsed: list):
        ...
        st = self._controller.tree_state
        if st.get("say"):
            self.say(st["say"])
            st["say"] = None
        self._last_heard_msg = None
    ...
    def run(self, start_pos: tuple):
        ...
        print(
            ...
            f"позиция: {start_pos}, вратарь: {self._is_goalie}"
        )
        while self._running:
            data = self._channel.read_next()
            if data:
